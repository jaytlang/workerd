...
static void
readline(uint32_t key)
{
	if (key != TEST_KEY) errx(1, "got error from unknown vm");

	warnx("injecting bad opcode");
	vm_injectfile(vm_fromkey(key), "urmom.txt", "69", 2);
}
...
fail(uint32_t key, char *msg)
{
	...
	if (strstr(msg, "VMMonitorBugException") == NULL)
		errx(1, "error doesn't look like a VM monitor bug exception");
...
ackdone(uint32_t key)
{
	...
	else errx(1, "successfully terminated vm when shouldn't have");
}
